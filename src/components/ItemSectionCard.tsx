...
const ItemSectionCard: React.FC<BuildCardProps> = ({ item }) => {
  return (
    <Link to={`/services/${item.slug}`} className="group relative h-[220px] sm:h-[260px] md:h-[280px] cursor-pointer overflow-hidden bg-gray-200 shadow-sm">
      ...
      <div className="absolute bottom-4 left-0 z-20 bg-black px-4 py-2 text-sm font-medium text-white transition-all duration-300 group-hover:translate-y-2 group-hover:opacity-0">
        {item.title}
      </div>
